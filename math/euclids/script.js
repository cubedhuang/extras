const debug = new Vue({
	...
	methods: {
		...
		checkNumbers() {
			...
			if (!d1.floor().eq(d1) || !d2.floor().eq(d2)) return "Make sure both numbers are integers!";
			if (!d1.abs().eq(d1) || d1.eq("0") || !d2.abs().eq(d2) || d2.eq("0")) return "Make sure both numbers are positive!";
			else return null;
		}
	}
});